...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(()=>{
    ...
    fetch('/api/auth',{headers:{Authorization:`Bearer ${token}`}})
      .then(r=>{if(r.ok){setAuthed(true);setReady(true);}else{localStorage.removeItem('admin_token');router.push('/admin/login');}})
      ...
      <aside className="w-56 hidden md:flex flex-col" style={{background:'rgba(255,255,255,0.02)',borderRight:'1px solid rgba(255,255,255,0.06)',minHeight:'100vh'}}>
        <div className="p-5" style={{borderBottom:'1px solid rgba(255,255,255,0.06)'}}>
          <div className="flex items-center gap-2 mb-1">
            ...
            <div className="flex items-baseline gap-1">
              ...
              <span className="text-sm font-black" style={{color:'#c084fc'}}>PAINTBALL</span>
            </div>
          </div>
          ...
          {nav.map(item=>{
            ...
            return (
              <Link key={item.href} href={item.href}
                ...
                style={active?{background:'rgba(168,85,247,0.15)',color:'#c084fc',border:'1px solid rgba(168,85,247,0.2)'}:{color:'#4b5563',border:'1px solid transparent'}}>
        ...
        <div className="p-4" style={{borderTop:'1px solid rgba(255,255,255,0.06)'}}>
          ...
          <button onClick={()=>{localStorage.removeItem('admin_token');router.push('/admin/login');}}
            className="text-xs transition-colors w-full text-left" style={{color:'#374151'}}>Выйти</button>
        </div>
      </aside>

      {/* Mobile bottom nav */}
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 flex justify-around py-2" style={{background:'rgba(6,10,18,0.95)',backdropFilter:'blur(16px)',borderTop:'1px solid rgba(255,255,255,0.06)'}}>
        {nav.map(item=>{
          const active = pathname===item.href || (item.href!=='/admin' && pathname.startsWith(item.href));
          return (
            <Link key={item.href} href={item.href} className="flex flex-col items-center py-1 px-2 rounded-lg transition-colors" style={{color:active?'#c084fc':'#374151'}}>
